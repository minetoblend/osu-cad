...
export class EditorRoom {
  ...
  accept(client: Socket<ClientMessages, ServerMessages>, user: UserEntity, access: EditorSessionEntity) {
    const roomUser = new RoomUser(user, client, this.nextSessionId++, this);

    this.logger.log(`User ${user.username} joined { sessionId: ${roomUser.sessionId} }`);
  ...
  private handleDisconnect(user: RoomUser) {
    ...
    this.logger.log(`User ${user.user.username} left { sessionId: ${user.sessionId} }`);
  ...
  handleKick(kickedBy: RoomUser, userId: number, reason: string, ban: boolean) {
    ...
    for (const targetUser of users) {
      ...
      this.broadcast("userLeft", targetUser.getInfo(), ban ? "banned" : "kicked");
      targetUser.socket.disconnect();
    }
  }

  private handleCommands(roomUser: RoomUser, commands: VersionedEditorCommand[]) {
    this.hasUnsavedChanges = true;
    for (const command of commands) {
      const context = new CommandContext(this.beatmap, false, false, command.version);
