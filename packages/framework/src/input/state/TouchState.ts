...
{
  public readonly activeSources = new ButtonStates<TouchSource>();

  public readonly touchPositions = Array.from({ length: MAX_TOUCH_COUNT }, () => new Vec2());

  public getTouchPosition(source: TouchSource): Vec2 | null
  {
    return this.isActive(source) ? this.touchPositions[source] : null;
  }

  public isActive(source: TouchSource): boolean
  {
    return this.activeSources.isPressed(source);
  }

  public enumerateDifference(previous: TouchState): { deactivated: Touch[]; activated: Touch[] }
  {
