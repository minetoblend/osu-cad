...
export class Mat3
{
  ...
  public translateFromLeft(x: number, y: number)
  {
    // col2 += col0 * x + col1 * y;
  ...
  public translateFromRight(x: number, y: number)
  {
    // row0 += row2 * v.x
    this.m00 += this.m20 * x;
    this.m01 += this.m21 * x;
  ...
  public rotateFromLeft(angle: number)
  {
  ...
  public rotateFromRight(angle: number)
  {
    const cos = Math.cos(angle);
    const sin = Math.sin(angle);
  ...
  public scaleFromLeft(x: number, y: number)
  {
    this.m00 *= x;
    this.m10 *= x;
    this.m20 *= x;
  ...
  public scaleFromRight(x: number, y: number)
  {
    this.m00 *= x;
    this.m01 *= x;
    this.m02 *= x;
  ...
  public shearFromLeft(x: number, y: number)
  {
    const xy = x * y;
  ...
  public shearFromRight(x: number, y: number)
  {
    const xy = x * y;
  ...
  public apply(pos: Vec2, target: Vec2 = new Vec2())
  {
  ...
  public applyToQuad(quad: Quad, target: Quad = quad)
  {
  ...
  public copyToPixiMatrix(matrix: Matrix)
  {
    matrix.a = this.m00;
    matrix.b = this.m01;
    matrix.c = this.m10;
