...
export class DrawableZWaveSliderTool extends DrawableHitObjectPlacementTool<Slider> {
  constructor() {
    super();
  }
  ...
  protected onPlacementStart(hitObject: Slider) {
    ...
    this.bridgeAnchor = new Vec2(0.42, 0.08);
  }
  ...
  #onHandleDragged = (index: number, e: DragStartEvent) => {
    ...
    if (index === 1 || index === 2) {
      ...
      const anchor = anchorPos.rotate(-angle).divF(length);

      this.bridgeAnchor = new Vec2(
        clamp(anchor.x, 0.01, 0.5),
        clamp(anchor.y, -0.5, 0.5),
      );
    }
