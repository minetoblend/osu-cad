...
export class BetterGlowFilter extends BlurFilter {
  constructor(options: GlowOptions) {
    ...
    this.#blendPass = new Filter({
      ...
      resources: {
        ...
        blendUniforms: {
          ...
          uOriginalAlpha: { value: 1, type: 'f32' },
        },
      },
    });
    ...
    this.drawOriginal = options.drawOriginal ?? true;
  ...
  set drawOriginal(value) {
    if (value !== this.#drawOriginal)
      this.#blendUniforms.uOriginalAlpha = value ? 1 : 0;

    this.#drawOriginal = value;
  }

  #blendUniforms: {
    uColor: Float32Array;
    uAlpha: number;
    uOriginalAlpha: number;
  };
  ...
  override apply(filterManager: FilterSystem, input: Texture, output: RenderSurface, clearMode: boolean) {
    ...
    this.#blendPass.resources.uGlow = blurOutput.source;
    this.#blendPass.apply(filterManager, input, output, clearMode);
