...
export class CursorContainer extends Drawable {
  ...
  removeUserCursor(userId: UserId) {
    const cursor = this.children.find(c => (c as RemoteCursor).user.id === userId);
    if (cursor) {
...
class RemoteCursor extends Drawable {
  ...
  onLoad() {
    ...
    this.username = this.addChild(new BitmapText({
  ...
  onActivityReceived(sessionId: number, activity: UserActivity) {
    ...
    if (activity.type === "composeScreen") {
      ...
      gsap.to(this, {
        alpha: animate(Math.abs(activity.currentTime - this.editor.clock.currentTimeAnimated), 500, 1500, 1, 0),
        duration: 0.05,
      });
    } else {
