...
import {IconButton, IconButtonOptions} from "@/editor/drawables/IconButton.ts";
...
export class ButtonPanel extends Component {
  ...
  onLoad() {
    watchEffect(() => {
      this.undoButton.disabled = !this.editor.commandManager.history.canUndo.value
      this.redoButton.disabled = !this.editor.commandManager.history.canRedo.value
    })
...
export class ButtonPanelButton extends IconButton {
  ...
  _onUpdate(point?: ObservablePoint) {
    ...
    this.background.clear()
      .roundRect(0, 0, this.size.x, this.size.y, 5)
      .fill({
        color: 0x1A1A20,
        alpha: 0.5,
      })
