...
export class TimelineObject extends Drawable {
  ...
  getPositionForTime(time: number, visibility: TimelineVisibility) {
    return (
      ((time - visibility.startTime) /
        (visibility.endTime - visibility.startTime)) *
      visibility.width
    );
  }
  ...
  constructor(
    ...
    this.x = this.timeline.positionManager.getPositionForTime(
      hitObject.startTime,
    );
  ...
  private installListeners() {
    ...
    this.endCircle.onpointerdown = (evt) => {
      ...
      if (hitObject instanceof Slider && !evt.shiftKey) {
        ...
        this.onglobalpointermove = (evt) => {
          const pos = evt.getLocalPosition(this.timeline);
          const time =
            pos.x / this.positionManager.pixelsPerMillisecond +
            this.positionManager.startTime;

          const targetDuration = time - hitObject.startTime!;
          const repeats =
            Math.round(targetDuration / hitObject.spanDuration) - 1;
        ...
        const onMove = (evt: FederatedPointerEvent) => {
          ...
          let time =
            pos.x / this.positionManager.pixelsPerMillisecond +
            this.positionManager.startTime;
          time = this.editor.beatmapManager.controlPoints!.snap(
            time,
            this.beatInfo.beatSnap,
          );
          ...
          const velocityOverride =
            ((hitObject.velocity / hitObject.baseVelocity) *
              hitObject.duration) /
            targetDuration;
        ...
        const onMove = (evt: FederatedPointerEvent) => {
          ...
          const endTime = this.editor.beatmapManager.controlPoints.snap(
            targetEndTime,
            this.beatInfo.beatSnap,
          );
    ...
    this.svText.on('pointerdown', (evt) => {
      if (evt.button === 2) {
        this.editor.commandManager.submit(
          updateHitObject(this.hitObject!, {
            velocity: null,
          }),
        );
      }
  ...
  onLoad() {
    if (this.hitObject) {
      ...
      const { off: offSelected } = this.editor.selection.hitObjectSelected.on(
        (hitObject) => {
      ...
      const { off: offDeselected } =
        this.editor.selection.hitObjectDeselected.on((hitObject) => {
          if (hitObject === this.hitObject) {
  ...
  set hitObject(hitObject: HitObject | undefined) {
    if (this._hitObject)
      this._hitObject.onUpdate.removeListener(this.onHitObjectUpdate);
  ...
  onTick() {
    ...
    this.endCircle.x =
      this.positionManager.getPositionForTime(endTime) - this.x;
    for (const child of this.repeats.children) {
      const circle = child as TimelineCircle;
    ...
    this.endCircle.eventMode =
      endTime > hitObject.startTime ? 'dynamic' : 'none';
...
class TimelineCircle extends Drawable {
  ...
  private readonly hitcircleoverlay = new Sprite(
    Assets.get('hitcircleoverlay'),
  );
