...
import {PopoverContainer} from "@/editor/drawables/menu/PopoverContainer.ts";
...
export class ControlPointTimeline extends Component {
  ...
  private readonly previewPoint = new ControlPointMarker(new ControlPoint({
    id: '',
    time: 0,
    timing: null,
    ...
    onEditorKeyDown(evt => {
      if (evt.ctrlKey && evt.key === 'p') {
        ...
        const controlPoint = new ControlPoint({
          ...
          time,
          velocityMultiplier: null,
          timing: this.editor.beatmapManager.controlPoints.timingPointAt(time).timing,
        })
        this.editor.commandManager.submit(EditorCommand.createControlPoint({
          controlPoint: controlPoint.serialize()
        }))
        this.editor.commandManager.commit();
        const createdControlPoint = this.editor.beatmapManager.controlPoints.getById(controlPoint.id);
        ...
        const controlPoint = new ControlPoint({
          id,
          time,
          velocityMultiplier: this.editor.beatmapManager.controlPoints.getVelocityAt(time),
          timing: null,
        })
        this.editor.commandManager.submit(EditorCommand.createControlPoint({
          controlPoint: controlPoint.serialize()
  ...
  onTick() {
    ...
    const timingPoints: ControlPoint[] = controlPoints.controlPoints.filter(it => it.time >= startTime && it.time <= endTime);
    ...
    if (this.mousePos) {
      ...
      this.canPlace = !controlPoints.controlPoints.some(it => Math.abs(it.time - time) < 0.01)
