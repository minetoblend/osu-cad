...
export class HitSoundPlacementTool extends Component {
  ...
  onpointerenter = (e: FederatedPointerEvent) => this.mousePos = Vec2.from(e.getLocalPosition(this));
  onpointerleave = () => this.mousePos = undefined;

  onTick() {
    if (this.mousePos) {
      this.previewSample.visible = true;
      const time = Math.round(this.beatInfo.snap(this.getTimeAtX(this.mousePos.x)));
      const layer = Math.floor((this.size.y - this.mousePos.y) / 20);
      this.insertPosition = { time, layer };
      this.previewSample.position.set(this.getXAtTime(time), this.size.y - layer * 20 - 10);
    } else {
  ...
  onpointerdown = (e: FederatedPointerEvent) => {
    if (e.button === 0 && this.insertPosition) {
      ...
      const layer = hitSounds.layers[this.insertPosition.layer];
      if (!layer) return;
      const sample = layer.samples.find(s => s.time === this.insertPosition.time);
      if (!sample) return;
