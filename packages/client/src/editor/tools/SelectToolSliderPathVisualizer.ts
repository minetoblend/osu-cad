import {SliderPathHandle, SliderPathVisualizer} from "./SliderPathVisualizer.ts";
  ...
  installHandleListeners(handle: SliderPathHandle, index: number) {
    handle.onpointerdown = (e) => {
      ...
      if ((e.button === 0 && e.ctrlKey) || isDoubleTap) {
        ...
        this.editor.commandManager.submit(updateHitObject(this.slider!, {
          path: controlPoints,
        }), true);
      } else if (e.button === 0) {
        const drag = () => {
          handle.onglobalpointermove = (e) => {
            ...
            const controlPoints = this.slider!.path.controlPoints.map(it => ({...it}));
