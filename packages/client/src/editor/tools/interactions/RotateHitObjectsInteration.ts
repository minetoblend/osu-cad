...
export class RotateHitObjectsInteraction extends UndoableInteraction {
  constructor(
    ...
    this.currentAngle = Math.atan2(
      tool.mousePos.y - center.y,
      tool.mousePos.x - center.x,
    );
  ...
  onTick() {
    ...
    if (this.aroundCenter && this.altDown) {
      ...
      const newBounds = Rect.containingPoints(
        getHitObjectPositions([...this.selection.selectedObjects]),
      )!;
    ...
    if (
      (!typedChars.startsWith('-') && typedChars.length > 0) ||
      typedChars.length > 1
    ) {
  ...
  private updateOverlay(
    ...
    this.overlayText.text = `${
      Math.round((((currentAngle - startAngle) * 180) / Math.PI) * 10) / 10
    }°`;
  }
