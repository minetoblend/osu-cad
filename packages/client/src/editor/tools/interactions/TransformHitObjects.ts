...
import {snapSliderLength} from "@/editor/tools/snapSliderLength.ts";
import {BeatInfo} from "@/editor/beatInfo.ts";

export function transformHitObjects(editor: EditorContext, container: Container, beatInfo: BeatInfo) {
  ...
  const onMove = (evt: TouchEvent) => {
    ...
    const angle = Math.atan2(t2.y - t1.y, t2.x - t1.x) - Math.atan2(p2.y - p1.y, p2.x - p1.x);
    ...
    for (const hitObject of editor.selection.selectedObjects) {
      ...
      if (hitObject instanceof Slider) {
        const pointTransform = transform.clone().translate(-transform.tx, -transform.ty);
