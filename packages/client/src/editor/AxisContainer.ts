...
export class AxisDrawable extends Drawable {
      ...
      if (evt.button === 2) {
        ...
        if (this.editor.selection.size > 0) {
          items.push({
            ...
            action: () => {
              mirrorAlongAxis([...this.editor.selection.selectedObjects], this.position0, this.position1, this.editor);
            },
          });
        }
  ...
  onAxisUpdate() {
    ...
    this.rotation = Math.atan2(position1.y - position0.y, position1.x - position0.x);
