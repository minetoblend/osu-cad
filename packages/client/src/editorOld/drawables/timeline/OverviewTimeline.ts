...
export class OverviewTimeline extends Component {
  ...
  currentTimeIndicator = new Graphics().circle(0, 0, 5).fill(0xffffff);
  ...
  onLoad() {
    ...
    const onUserJoined = (user: UserSessionInfo) => {
      if (
        user.sessionId === this.editor.connectedUsers.ownUser.value?.sessionId
      )
        return;
  ...
  private setTimeFromPosition(position: number) {
    const progress =
      (position - this.barHeight * 0.5) / (this.size.x - this.barHeight);
  ...
  onTick() {
    ...
    this.currentTimeIndicator.position.set(
      this.barHeight * 0.5 + progress * availableWidth,
      this.size.y * 0.5,
    );
  ...
  updateActiveTime() {
    ...
    for (let i = 1; i < hitObjects.length; i++) {
      ...
      if (
        hitObject.startTime - lastObjectEnd > threshold ||
        i === hitObjects.length - 1
      ) {
        ...
        this.activeTimeOverlay
          .roundRect(
            this.barHeight * 0.5 +
              (start / this.editor.clock.songDuration) *
                (this.size.x - this.barHeight),
            (this.size.y - this.barHeight) * 0.5,
            ((end - start) / this.editor.clock.songDuration) *
              (this.size.x - this.barHeight),
            this.barHeight,
            this.barHeight * 0.5,
          )
    ...
    const onUserActivity = (sessionId: number, activity: UserActivity) => {
      if (sessionId === this.user.sessionId) {
        if (activity.type === 'composeScreen') {
          ...
          this.position.x =
            parent.barHeight * 0.5 +
            (activity.currentTime / this.editor.clock.songDuration) *
              (parent.size.x - parent.barHeight);
        } else {
          this.visible = false;
        }
      }
