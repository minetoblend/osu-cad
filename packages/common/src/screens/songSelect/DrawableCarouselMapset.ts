import type { HoverLostEvent, ReadonlyDependencyContainer, Vec2 } from 'osucad-framework';
import type { ImageSource } from 'pixi.js';
import type { CarouselBeatmapSetInfo } from './CarouselBeatmapSetInfo';
import { almostEquals, Anchor, Axes, Container, DrawableSprite, EasingFunction, FastRoundedBox, FillMode, lerp, resolved, RoundedBox } from 'osucad-framework';
...
class MapsetBackground extends Container {
  constructor(readonly mapset: CarouselBeatmapSetInfo) {
    super({
      relativeSizeAxes: Axes.Both,
    });
  }
  ...
  async loadBackground() {
    if (this.findClosestParentOfType(BeatmapCarousel)!.isFastSeeking) {
