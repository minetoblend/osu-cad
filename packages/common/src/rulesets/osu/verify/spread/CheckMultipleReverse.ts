      ...
      if (difficulty === DifficultyType.Easy) {
        if (slider.duration < problemThreshold) {
          yield this.createIssue({
            level: 'problem',
            message: 'This slider is way too short to have multiple reverses.',
            timestamp: slider,
            cause: 'A slider has at least 2 reverses and is 250 ms or shorter (240 bpm 1/1) in an Easy, ' + 'or 125 ms or shorter (240 bpm 1/2) in a Normal.',
          });
        }
        else if (slider.duration < warningThreshold) {
          yield this.createIssue({
            level: 'warning',
            message: 'This slider is very short to have multiple reverses.',
            timestamp: slider,
            cause: 'A slider has at least 2 reverses and is 333 ms or shorter (180 bpm 1/1) in an Easy, ' + 'or 167 ms or shorter (180 bpm 1/2) in a Normal.',
          });
        }
      }
