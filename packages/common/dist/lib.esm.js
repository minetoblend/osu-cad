import { variantModule, fields, payload, lookup } from 'variant';
...
const ServerMessage = variantModule({
  ...
  chatMessage: fields(),
  userActivity: fields(),
  userRoleChanged: fields()
});

const ClientMessage = variantModule({
  kickUser: fields(),
...
class SliderPath {
  ...
  calculateSegment(type, start, end) {
    const points = this.controlPoints.slice(start, end + 1).map((p) => new Vector2(p.x, p.y));
    switch (type) {
...
const EditorCommand = variantModule({
  updateHitObject: fields(),
  createHitObject: fields(),
  deleteHitObject: fields(),
  createBookmark: fields(),
  removeBookmark: fields()
});
...
const UserActivity = variantModule({
  idle: fields(),
