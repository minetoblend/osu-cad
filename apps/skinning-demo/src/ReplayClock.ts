import type { IClock, IFrameBasedClock } from "@osucad/framework";
import { type FrameTimeInfo, isFrameBasedClock, StopwatchClock } from "@osucad/framework";

const fps_calculation_interval = 250;

export class ReplayClock implements IFrameBasedClock
{
  source!: IClock;
  ...
  processFrame(customTime?: number): void
  {
    this.#betweenFrameTimes[this.#totalFramesProcessed % this.#betweenFrameTimes.length] = this.currentTime - this.lastFrameTime;
    this.#totalFramesProcessed++;

    if (customTime !== undefined && this.#processSource && isFrameBasedClock(this.source))
    {
    ...
    this.currentTime = customTime ?? this.sourceTime;
    this.#timeInfo = {
