...
export class DrawableHitCircle extends DrawableHitObject {
    ...
    init() {
        ...
        this.#circlePiece.tint = this.ctx.beatmap.metadata.getComboColor(circle.comboIndex.value)
    ...
    update(time: number) {
        ...
        if (circle.selectedBy.value) {
            this.selectionOverlay.scale.set(
                this.ctx.beatmap.difficulty.circleRadius / 64 * 1.1
            )
            if (circle.selectedBy.value === this.ctx.users.sessionId) {
                this.selectionOverlay.alpha = 1
            } else {
                this.selectionOverlay.alpha = this.#circlePiece.alpha
            }

            const color = this.ctx.users.getColor(circle.selectedBy.value)
            this.selectionOverlay.tint = color.hex
